...
export class LoginComponent implements OnInit, AfterViewInit {
  ...
  constructor(
    ...
    this.loginFields = this.formBuilder.group({
      email: [
        "",
        Validators.compose([Validators.required, FormValidator.validateEmail]),
      ],
          ...
          (response) => {
            ...
            if (this.userStatus == "deleted") {
              ...
              if (response.businesses.length === 1) {
                this.dataService
                  .update_user_config(this.authService.currentUser.user_id, {
                    active_business_id: response.businesses[0].id,
                  })
                  .subscribe((response) => {
              ...
              if (this.businessId) {
                GrowlService.message(
                  "Job application applied successfully.",
                  "success"
                );
              }
            }
  ...
  shouldDisplayRequired(path: Array<string>): any {
    if (
      this.loginFields.get(path).hasError("required") &&
      this.loginFields.get(path).touched
    ) {
  ...
  shouldDisplayEmailValidation(field: Array<string>, error: string): any {
    if (
      this.loginFields.get(field).hasError(error) &&
      this.loginFields.get(field).touched
    ) {
  ...
  checkV(e) {
    ...
    if (key === 32) {
      this.rememberCheckbox.nativeElement.checked =
        !this.rememberCheckbox.nativeElement.checked;
    }
    if (key === 13) {

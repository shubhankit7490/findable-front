...
export class SearchComponent implements OnInit, AfterViewInit {
  ...
  @ViewChild("candidateTypeSelector")
  candidateTypeSelector: ArraySelectorComponent;
  @ViewChild("purchaseApplicantBlock")
  purchaseApplicantBlock: PurchaseApplicantComponent;
  ...
  @ViewChild("preferencesModalForm")
  preferencesModalForm: PreferencesFormComponent;
  @ViewChildren(NoteComponent) noteComponents: QueryList<NoteComponent>;
  @ViewChildren(PreferencesFormComponent)
  preferencesFormComponent: QueryList<PreferencesFormComponent>;
  ...
  private mapSearchData(
    data: models.ApplicantsSearchProfileParsed | models.ApplicantsSearchProfile
  ): models.ApplicantsSearchProfileParsed | models.ApplicantsSearchProfile {
  ...
  private parseEducationLevel(levels: models.DictionaryItem[]) {
    if (!levels) {
      ...
      this.selectedEducationLevel = levels.filter(
        ({ id }: models.DictionaryItem): boolean => id !== null
      );

      let educationLevelFreeText = levels.filter(
        ({ id }: models.DictionaryItem): boolean => id === null
      )[0];
  ...
  public onPositionTypeSelected(event) {
    this.searchModel.position_type =
      event.item.id === "null" ? null : event.item.name;
  }
  ...
  public onSenioritySelection(event) {
    this.searchModel.seniority =
      event.item.id === "null" ? null : event.item.id;
  }
  ...
  public onResponsibilityRemoved(id: number) {
    this.searchModel.areas_of_focus = this.searchModel.areas_of_focus.filter(
      (value: models.DictionaryItem): boolean => {
        return id !== value.id;
      }
    );
  }
  ...
  public onEducationSelected(item?: models.DictionaryItem) {
    ...
    this.searchModel.education_level = this.selectedEducationLevel.map(
      (value, index) => value
    );
  }

  public onEducationLevelRemove(id: number) {
    this.selectedEducationLevel = this.selectedEducationLevel.filter(
      (value) => Number(id) !== Number(value.id)
    );
  ...
  public onTraitSelection(event?: models.Trait) {
    this.tmpTrait = "";
    this.searchModel.traits = this.selectedTraits.map(
      (value, index): models.DictionaryItem => value
    );
  }
  ...
  public onSalaryPeriodSelected(value, doSearch: boolean) {
    if (value === "null" || value == "Period") {
      this.searchModel.salary_from.salary_period =
        this.searchModel.salary_to.salary_period = null;
    } else {
      this.searchModel.salary_from.salary_period =
        this.searchModel.salary_to.salary_period = value;
    }
  ...
  public onUploadedDateSelected(event) {
    ...
    this.searchModel.uploaded_date =
  ...
  public onUploadedusertypeSelected(typeObject: EmitArraySelector) {
    ...
    this.searchModel.user_type =
      typeObject.item.id === "null" ? null : typeObject.item.id;
    this.performSearch();
  }
  ...
  public onActionSelected(event) {
    switch (event.item.id) {
      ...
      case "deleteandremove":
        if (
          confirm("Are you sure want to delete and remove from market place")
        ) {
  ...
  public onPurchase(profile: OnPurchaseData) {
    ...
    if (profile) {
      this.messageService.sendMessage({
        name: "RELOAD_PROFILE",
        data: profile.data.userId,
      });
    }
    this.dataService
      .profile_get(profile.data.userId, 0)
      .subscribe((response) => {
        let value = this.table.value.find(
          (value) => value.id == profile.data.userId && value
        );
  ...
  public onchnagestatus(userid) {
    ...
    this.dataService.profile_get(userid, 1).subscribe((response) => {
        ...
        .subscribe(
          (response: models.ApplicantsSearchResultProfiles) => {
            ...
            this.location.replaceState(
              `/business/search?token=${response.token}`
            );
          },
          (error) => {
            this.loadingResults = false;
  ...
  public loadSavedSearch(
    savedSearch: models.ApplicantsSearchProfileParsed
  ): void {
  ...
  private setAvarageExperience(
    collection: models.ApplicantsSearchResultProfile[] = []
  ) {
    let avg = 0;
  ...
  private setAvarageMonthlySalary(collection = []) {
    ...
    this.avgSalary =
      annual > 1
        ? this.formatNumber(Number.parseFloat((annual / count).toFixed(1)), ",")
        : 0;
    this.totalSalary = this.formatNumber(
      Number.parseFloat(total.toFixed(1)),
      ","
    );
    this.avgSalaryMonthly =
      sum > 1
        ? this.formatNumber(Number.parseFloat((sum / count).toFixed(1)), ",")
        : 0;
  }
  ...
  openprefrencesmodel(userid, status) {
    ...
    this.preferences$.subscribe(
      (response: UserPreferencesExt) => {
        ...
        // Update the tourService
        this.tourService.collect("status", response.employment_status === null);
        this.tourService.collect(
          "status_edit",
          response.employment_status === null
        );
        this.onLoaded.emit(true);

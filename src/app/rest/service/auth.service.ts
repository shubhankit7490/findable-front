...
export class AuthService {
  ...
  getUserId() {
    if (
      this.currentUser.hasOwnProperty("key") &&
      this.currentUser["active_user_id"] &&
      ...
      switch (user["role"]) {
        ...
        case "manager":
          if (user.status === userActive || user.status === userPending) {
            ...
            if (user["business_id"]) {
              this.dataService
                .get_business_setup_status(user.business_id)
                .subscribe(
                  (responseBizRecord) => {
                    ...
                    if (responseBizRecord.status === Setup) {
                      ...
                      responseBizRecord.status === ActiveBiz ||
                      user.status === PendingBiz
                    ) {
                      this.router.navigate([
                        this.redirectUrl || "/business/search",
                      ]);
                    } else {
                      this.logout();
                      this.router.navigate(["/user/login"]);
                  ...
                  (error) => {
                    console.log(
                      "@authService > checkUserStatus [ERROR]:",
                      error
                    );
                  }
                );
        ...
        case "admin":
          if (!!user["active_user_id"]) {
            ...
            this.dataService
              .get_business_setup_status(user["active_business_id"])
              .subscribe(
                (responseBizRecord) => {
                  this.currentBusiness = responseBizRecord;
                  if (responseBizRecord.status === Setup) {

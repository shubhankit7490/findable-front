...
export class PreferencesFormComponent implements OnInit, OnChanges {
  ...
  @ViewChild("locationOfInteresetComponent")
  locationOfInteresetComponent: LocationsFormComponent;
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    this.preferences.subscribe((response: extModels.UserPreferencesExt) => {
      ...
      response.desired_salary_period =
        response.desired_salary_period == null
          ? "Y"
          : response.desired_salary_period;
      const {
    ...
    if (this.prefrence == "edit") {
      ...
      this.dataService
        ...
        .subscribe((response: any) => {
          ...
          this.sendModelExperience.areas_of_focus =
            this.recentJob.areas_of_focus;
  ...
  public onSubmit(event: Event) {
    ...
    if (
      !!this.preferencesModel.desired_salary &&
      !this.preferencesModel.desired_salary_period
    ) {
    ...
    this.response$.subscribe(
      (response: Response) => {
        ...
        if (desiredSalary !== null) {
          this.preferencesModel.desired_salary = desiredSalary
            .replace(" ", "")
            .replace("$", "")
            .replace(/,/g, "");
        } else {
  ...
  private updateusernote() {
    ...
    if (data != "") {
      this.dataService
        .user_note_put(this.user_id, data, this.form.get("type").value)
        .subscribe((response) => {});
    }
  }

  private prepareModel() {
    this.preferencesModel.employment_status =
      this.form.get("employment_status").value;
    this.preferencesModel.current_status =
      this.form.get("current_status").value;
    this.preferencesModel.employment_type =
      this.form.get("employment_type").value;
    this.preferencesModel.desired_salary = this.form
      .get("desired_salary")
      .value.replace("$", "")
      .replace(" ", "")
      .replace(",", "")
      .replace(/,/g, "");
    this.preferencesModel.desired_salary_period = this.form.get(
      "desired_salary_period"
    ).value;
    this.preferencesModel.benefits = this.form.get("benefits").value;
    this.preferencesModel.only_current_location =
      this.form.get("relocation").value == true;
    this.preferencesModel.relocation =
      this.form.get("relocation").value == true;
    this.preferencesModel.available_from =
      this.form.get("available_from").value;
  ...
  onDatePickerToggle(event: Event) {
    if (this.datePickerOpened) {
      ...
      this.datePickerLeft =
        datePickerButton.offsetLeft + datePickerButton.offsetWidth - 300;
  ...
  handleErrors(error) {
    ...
    if (error.status === 406) {
      for (let i = 0; i < err_body.message.length; i++) {
        this.formErrors[err_body.message[i].field] =
          err_body.message[i]["error"];
      }
    }
  ...
  public onSelectSeniority(event: Event): void {
    ...
    let name = (event.target as HTMLSelectElement).options[
  ...
  private observeForm() {
    this.form
      .get("current_status")
      .valueChanges.subscribe((data: extModels.UserPreferencesExt) => {
        if (!!data) {
          if (
            data.current_status === "not looking" &&
            this.form.controls["available_from"].enabled
          ) {
